...
The built image has size **300MB**
...
b3d7ce3bfb28   About a minute ago   ENTRYPOINT ["/usr/local/bin/yt-dlp"]            0B
<missing>      About a minute ago   USER appuser                                    0B
<missing>      About a minute ago   RUN /bin/sh -c apt-get update && apt-get ins…   110MB
  ...
```
...
Size of the resulting image is **82.9MB**
...
We build this file with `:alpine-3.21` as the tag:
...
From the history, we can see that our single `RUN` layer size is 51.9MB

```sh
$ docker image history yt-dlp:alpine-3.21

IMAGE          CREATED              CREATED BY                                      SIZE     
6977f69b15e0   About a minute ago   ENTRYPOINT ["/usr/local/bin/yt-dlp"]            0B
<missing>      About a minute ago   USER appuser                                    0B
<missing>      About a minute ago   RUN /bin/sh -c apk add --no-cache curl pytho…   51.9MB
<missing>      About a minute ago   WORKDIR /mydir                                  8.19kB
```

So in total, our Alpine variant is about 82.9MB megabytes, significantly less than our Ubuntu-based image.
...
The resulting image size is **84.1MB** so it is slightly larger than the previous one where we installed Python by ourselves.

Back in chapt 2, we published the Ubuntu version of yl-dlp with the tag _latest_.
...
Before continuing, let us build and start the image to see that we are on right track.

When we run docker build -t jekyll . && docker run -p 4000:4000 jekyll and go to http://localhost:4000 the template site can be seen:
...
## Exercises 3.8 - Multi-stage frontend
